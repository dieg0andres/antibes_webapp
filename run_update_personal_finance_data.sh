#!/bin/bash
# run_update_personal_finance_data.sh
...
LOG_FILE="$LOG_DIR/update_personal_finance_data.log"
...
{
  echo "=== $(date -u +'%Y-%m-%dT%H:%M:%SZ') START update_personal_finance_data ==="
  if python "$PROJECT_DIR/manage.py" update_personal_finance_data ; then
    echo "=== $(date -u +'%Y-%m-%dT%H:%M:%SZ') SUCCESS update_personal_finance_data ==="
  else
    echo "=== $(date -u +'%Y-%m-%dT%H:%M:%SZ') FAILURE update_personal_finance_data (exit code $? ) ==="
  fi
  echo ""
} >> "$LOG_FILE" 2>&1
